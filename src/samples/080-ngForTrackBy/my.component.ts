...
@Component({
  ...
  template: `
      ...
      <h3> Friends </h3>
      <ul>
        <li *ngFor="#friend of friends ; #index = index" nodeLogger="{{ friend.name }} at index {{ index }}.">
          {{ friend.name }}
        </li>
      </ul>
      <h3> Enemies </h3>
      <ul>
        <li *ngFor="#enemy of enemies ; #index = index ; trackBy:personIdentity" nodeLogger="{{ enemy.name }} at index {{ index }}.">
          {{ enemy.name }}
        </li>
      </ul>`
})
